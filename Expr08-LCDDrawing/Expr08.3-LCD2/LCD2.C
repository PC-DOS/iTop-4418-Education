	...
	unsigned char pixel;
        ...
        printf("bitcolorcount%d\n",bits_per_pixel); 
        //跳转的数据区  
        if (InfoHead.ciBitCount == 8) 
        {
	   fread( (char *)&aaa, 1024,1, fp );
        }          
        ...
        {  
            ...
            if (InfoHead.ciBitCount == 8) 
            {
	       rc = fread( &pixel, 1, 1, fp);  
            //if (rc != sizeof(PIXEL))  
                //break;  
              
      
            //显示每一个像素  
               *(fbp + location + 0)=pixel;  
               *(fbp + location + 1)=pixel;  
               *(fbp + location + 2)=pixel;  
               *(fbp + location + 3)=0; 
             }
             else
             {
                ...
                *(fbp + location + 1)=pix.red;
                *(fbp + location + 2)=pix.blue; 
                *(fbp + location + 3)=0; 
             }
